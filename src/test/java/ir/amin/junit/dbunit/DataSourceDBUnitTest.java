package ir.amin.junit.dbunit;

import static ir.amin.junit.dbunit.ConnectionSettings.JDBC_URL;
...
public class DataSourceDBUnitTest extends DataSourceBasedDBTestCase {
	...
	// LoggerFactory.getLogger(DataSourceDBUnitTest.class);
	...
	@Test
	public void givenDataSet_whenSelect_thenFirstTitleIsGreyTShirt()
			throws SQLException {
	...
	@Test
	public void givenDataSet_whenInsert_thenTableHasNewClient()
			throws Exception {
		...
		try {
			...
			final ITable actualData = getConnection().createQueryTable(
					"result_name",
	...
	public void givenDataSet_whenDelete_thenItemIsDeleted() throws Exception {
		...
		try {
			final InputStream is = DataSourceDBUnitTest.class.getClassLoader()
					.getResourceAsStream("dbunit/items_exp_delete.xml");
			ITable expectedTable = (new FlatXmlDataSetBuilder().build(is))
					.getTable("ITEMS");
	...
	public void givenDataSet_whenUpdate_thenItemHasNewName() throws Exception {
		...
		try {
			...
			connection.createStatement().executeUpdate(
					"update ITEMS set title='new name' where id = 1");
	...
	@Test
	public void givenDataSet_whenInsertUnexpectedData_thenFailOnAllUnexpectedValues()
			throws Exception {
		try {
			final InputStream is = getClass().getClassLoader()
					.getResourceAsStream(
							"dbunit/expected-multiple-failures.xml");
			...
			final ITable actualData = getConnection().createDataSet().getTable(
					"ITEMS");
			...
			// .getDiffList()
			// .stream()
			// .map(d -> formatDifference((Difference)
			// d)).collect(joining("\n"));
